...
const sendOTP = async (req, res) => {
  ...
  if (!user) {
    return res
      .status(401)
      .json({ error: "This Email Address is not Registered with US" });
  }
  ...
  if (sendMail === "error") {
    return res
      ...
      .json({
        error:
          "We are not able to send the OTP to the given Email Address. Try again afetr some Time",
      });
  }
...
const updateFullname = async (req, res) => {
  ...
  if (!user) {
    return res
      .status(401)
      .json({ error: "Some Internal Error Please Try agin after Some Time" });
  }
  console.log(user);
