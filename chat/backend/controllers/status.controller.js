  ...
  try {
    ...
    if (alreadyStatus) {
      if (alreadyStatus.statusData.length > 5) {
        return res
          .status(301)
          .json({ error: "You can upload only upto 5 files" });
      }
...
const getUserStatus = async (req, res) => {
  const userId = req.user?._id;
  const userStatus = await Status.findOne({ statusId: userId })?.populate({
    path: "statusId",
...
const getAllUsersStatus = async (req, res) => {
  const alluserStatus = await Status.find({}).populate({
    path: "statusId",
    select: "-password -email",
  });
