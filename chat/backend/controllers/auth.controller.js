...
export const generateOTP = () => {
  ...
  console.log("authenticateOTP", authenticateOTP);
  return value;
};
...
const signUpDetailsVerification = async (req, res) => {
  ...
  if (seeemail) {
    return res.status(400).json({
      error: "This Email Id already Exists. Please Provide a different One",
    });
  }
  ...
  if (mailVerification == "error") {
    return res.status(400).json({
      error:
        "There is Some Problem with the Provided Email Address. We are not Able to Verify it",
    });
  }
  return res
    .status(200)
    .json({ message: "All Details are Verified Except The Email" });
};
...
const signup = async (req, res) => {
  try {
    ...
    console.log("OTP", OTP);
    console.log("authenticateOTP1 ", authenticateOTP);
    console.log(typeof OTP);
    console.log(typeof authenticateOTP);
...
export { login, logout, signup, signUpDetailsVerification };
